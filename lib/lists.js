...
export const loadList = (lists) => {
  lists.forEach((list) => {
    $('.user_lists').append(`<div class="userlist"><h4 class="list${list.id}">${list.name}</h4><button id = ${list.id} class='recipes${list.id} get-recipes'> Get Recipes</button><button id = ${list.id} class='delete${list.id} list-delete'> Delete</button></div>`)

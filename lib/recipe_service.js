...
export const showRecipes = (id, title, summary, source, url) => {
  if ($('ul').length < 3) {
    $('.recipe-top3').append(`<div class="recipe-card"><ul class="ul"><a class="recipe_url" href="${url}"><h3 class="title">${title}</h3></a><p>${summary}<p><p>${source}</p><button class="save-recipe${id}" label='save-recipe' type="button" value="Save Recipe">Save Recipe</button></ul></div>`);
  }
};

export const showAllRecipes = (id, title, summary, source, url) => {
  $('.recipes').append(`<div class="recipe-card"><ul><a href="${url}"><h2>${title}</h2></a><p>${summary}<p><p>${source}</p><button class="save-recipe${id}" label='save-recipe' type="button" value="Save Recipe">Save Recipe</button></ul></div>`)
}
